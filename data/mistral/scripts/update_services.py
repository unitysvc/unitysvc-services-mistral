#!/usr/bin/env python3
"""
Simplified update_services.py for Mistral using unitysvc_services data builders.
...
class ModelExtractor:
    ...
    def determine_service_type(self, model_id: str) -> str:
        ...
        if any(kw in model_lower for kw in ["rerank"]):
            return "rerank"
        if any(kw in model_lower for kw in ["vision", "pixtral"]):
    ...
    def build_listing(self, model_id: str) -> ListingDataBuilder:
        ...
        builder.set_raw("service_options", {"default_parameters": {"api_key": "${ secrets.MISTRAL_API_KEY }"}})
